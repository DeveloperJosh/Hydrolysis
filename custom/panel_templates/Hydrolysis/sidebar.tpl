<aside class="main-sidebar sidebar-dark-primary elevation-4">
    <a href="{$PANEL_INDEX}" class="brand-link">
        <span class="brand-text font-weight-light">	<strong>{$SITE_NAME}</strong></span>
    </a>
                ...
                {foreach from=$NAV_LINKS key=name item=item}
                    ...
                    {else}
                        ...
                        {else}
                            {* Normal link *}
                            <li class="nav-item">
                                <a class="nav-link{if $PAGE eq $name} active{/if}" href="{$item.link}" target="{$item.target}" button>
                                    {$item.icon}
                                    <p>
                                        {$item.title}
                                    </p>
